...
public class LockTest {
    ...
    static class Worker implements Runnable {
        ...
        public void run() {
            reentrantLock.lock();
            //reentrantLock.tryLock(3, TimeUnit.DAYS);
            try {
                if(reentrantLock.isHeldByCurrentThread())
                    System.out.printf("Thread %s entrou em sessão crítica%n", nome);
                System.out.printf("%d Threads esperando na fila%n", reentrantLock.getQueueLength());
                System.out.printf("Threads %s vai trabalhar por 2 segundos%n", nome);
                Thread.sleep(2000);
                System.out.printf("Thread %s finalizouo trabalho%n", nome);
